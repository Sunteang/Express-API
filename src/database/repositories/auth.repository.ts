...
//   // Function to initiate authentication (sign-in)
...
//   // Function to confirm sign up with confirmation code
...
export class AuthRepository {
  ...
  public async getAllUsers(page: number = 1, limit: number = 10) {
    try {
      ...
      const users = await User.find().skip(skip).limit(limit).select("-__v"); // Exclude the version key
